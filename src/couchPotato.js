...
(function () {
     ...
     * $couchPotatoProvider} documentation.
        ...
        function CouchPotatoProvider($controllerProvider,
                                     $compileProvider,
                                     $provide,
                                     $filterProvider) {
            ...
            //加载javascript脚本
            function loadScripts(script,returnIndex, returnSubId,callback){
                var taskFns = [];
                if(Array.isArray(script)){
                    script.forEach(function(js){
                        (function(js){
                            taskFns.push(function(callback){
                                var cache = self._cache_dependencies[js];
                                if(cache){
                                    callback(null,null);
                                    ...
                                    $.ajax({
                                        ...
                                        callback(null,out);
                                        self._cache_dependencies[js] = out;
                                    });
                                }
                            });
                        })(js);
                    ...
                    async.parallel(taskFns,function(err,results){
            ...
            function resolveDependencies(dependencies, returnIndex, returnSubId) {
                function delay($q, $rootScope) {
                    ...
                    var defer = $q.defer();
                    loadScripts(dependencies, returnIndex, returnSubId,function(err,results){

                        defer.resolve(results);

                        var isHasCache = false;

                        results.forEach(function(cache){
                            if(cache!=null){
                                isHasCache = true;
                                return false;
                            }
                    ...
                    //var cache = self._cache_dependencies[key];
                    //if(cache){
                    //    defer.resolve(cache);
                    //    //$rootScope.$apply();
                    //}else{
                    ...
                    //
                    //        defer.resolve(out);
                    //        self._cache_dependencies[key] = out;
                    //        $rootScope.$apply();
                    //    });
                    //}
    ...
    if (typeof(define) === 'function' && define.amd) {
        ...
        define(['angular'], function () {
            return new CouchPotato(window.angular);
        });
    }
    else {
